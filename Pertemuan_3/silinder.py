...
def hitung_luas_volume():
    try:
        # Ambil jari-jari dan tinggi silinder dari input pengguna
        jari_jari = float(jari_jari_entry.get())
        tinggi = float(tinggi_entry.get())
        
        # Hitung luas permukaan dan volume silinder
        luas_permukaan = 2 * math.pi * jari_jari * (jari_jari + tinggi)
        volume = math.pi * jari_jari**2 * tinggi
        
        # Update label hasil
        luas_permukaan_var.set(f"Luas Permukaan Silinder: {luas_permukaan:.2f}")
        volume_var.set(f"Volume Silinder: {volume:.2f}")
    except ValueError:
...
window = tk.Tk()
window.title("Menghitung Luas Permukaan dan Volume Silinder")
...
jari_jari_label = tk.Label(frame, text="Jari-Jari Silinder:")
jari_jari_label.grid(row=0, column=0, padx=10, pady=10, sticky="w")

tinggi_label = tk.Label(frame, text="Tinggi Silinder:")
